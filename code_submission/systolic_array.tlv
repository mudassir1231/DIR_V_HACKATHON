...
\SV
   ...
   m5_makerchip_module   // (Expanded in Nav-TLV pane.)
      ...
      assign failed = cyc_cnt > 20;
      assign x_inp = cyc_cnt;
...
\TLV
   ...
   |processing_element
      /rows_num[7:0]
         /columns_num[7:0]
            @1
               $x_in[7:0] = $reset ? 0 : (columns_num == 0) ? *x_inp : /top|processing_element/rows_num/columns_num[(#columns_num-1)%8]$x_out[7:0];
               $x_out[7:0] = (columns_num == 7) ? 0 : /top|processing_element/rows_num/columns_num[#columns_num]$x_in[7:0];
               $w_out[7:0] = (rows_num == 7) ? 0 : /top|processing_element/rows_num[#rows_num]$w_in[7:0];
               $w_in[7:0] = $reset ? 0 : (rows_num == 0) ? 8 : /top|processing_element/rows_num[(#rows_num -1 )%8]$w_out[7:0];
            
            
               $acc[15:0] = $w_in[7:0] * $x_in[7:0] + >>1$acc;
