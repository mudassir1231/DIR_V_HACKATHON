...
// Function to multiply a sub-matrix of input with the kernel and return the sum
int16_t multiplySubMatrix(int8_t input[INPUT_ROWS][INPUT_COLS], int8_t kernel[KERNEL_ROWS][KERNEL_COLS], int row_offset, int col_offset) {
    int16_t sum = 0;
    for (int m = 0; m < KERNEL_ROWS; ++m) {
        for (int n = 0; n < KERNEL_COLS; ++n) {
            sum += (int16_t)input[row_offset + m][col_offset + n] * (int16_t)kernel[m][n];
        }
    }
    return sum;
}
...
void Convolution2D(int8_t input[INPUT_ROWS][INPUT_COLS], int8_t kernel[KERNEL_ROWS][KERNEL_COLS], int8_t result[OUTPUT_ROWS][OUTPUT_COLS]) {
    for (int i = 0; i < OUTPUT_ROWS; ++i) {
        for (int j = 0; j < OUTPUT_COLS; ++j) {
            result[i][j] = (int8_t)multiplySubMatrix(input, kernel, i, j);
        }
    }
}
